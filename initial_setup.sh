...
GEM_BINARY=$(whereis gem | cut -d':' -f2 | tr -d ' ')
if [ "$GEM_BINARY" = "" ]; then
    ...
    if [ $(uname) = "FreeBSD" ]; then
        ...
        case $NAME in
            ...
            "Debian GNU/Linux"|"Ubuntu") apt-get update && apt-get install -y ruby ruby-devel make gcc;;
        esac
    fi
    GEM_BINARY=$(whereis gem | cut -d':' -f2 | tr -d ' ')
    if [ "$GEM_BINARY" = "" ]; then
...
BUNDLER=$(whereis bundle | cut -d':' -f2 | tr -d ' ')
$BUNDLER install
